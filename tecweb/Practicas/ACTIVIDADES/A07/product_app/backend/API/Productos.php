...
require_once 'API/database.php'; //USO DE NAMESPACES
...
class Productos extends DataBase{
    ...
    public function add($jsonOBJ){
        $this->response = array(
            'status'  => 'Error!!',
            'message' => 'Producto en existencia'
        );
    
        $data = json_decode($jsonOBJ->data, true); //DECODIFICA EL JSON EN UN ARREGLO ASOCIATIVO
    
        $sql = "SELECT * FROM productos WHERE nombre = '{$data['nombre']}' AND eliminado = 0";
        $result = $this->conexion->query($sql);

        if($result->num_rows > 0){
            $this->response['message'] = "¡¡Error!!, Producto ya existente.";
        }else{
